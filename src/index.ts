...
export class RingCentralCallControl extends EventEmitter {
  ...
  get sessions() {
    if (this._filerOutRingOutInboundLeg) {
      return Array.from(this._sessionsMap.values()).filter(
        (s: Session) => !s.isRingOutInboundLeg,
      );
    }
  ...
  private _filerOutRingOutInboundLeg: boolean;
  constructor({
    sdk,
    accountLevel,
    ...
    extensionInfo,
    userAgent,
    filerOutRingOutInboundLeg = false,
  }: {
    sdk: RingCentralSDK;
    ...
    userAgent?: string;
    filerOutRingOutInboundLeg?: boolean;
  }) {
    super();
    ...
    this._filerOutRingOutInboundLeg = filerOutRingOutInboundLeg;
    this.initialize();
  }
  ...
  public onNotificationEvent(message: SessionMessage) {
    ...
    if (!existedSession) {
      const disconnectedParties = newData.parties.filter(
      ...
      newData.isRingOutInboundLeg = areTwoLegs(newData, this.sessions);
  ...
  public async loadSessions(activeCalls) {
    ...
    try {
      await Promise.all(
        activeCalls.map(async (activeCall) => {
          const response = await this._sdk
            .platform()
            .get(
  ...
  public async createConference() {
    ...
    sessionData.extensionId = this.extensionId;
    sessionData.accountId = this.accountId;
    sessionData.parties = (sessionData.parties || []).map((p) =>
      formatParty(p),
    );
    const session = new Session(sessionData, this._sdk, this._accountLevel);
    this._sessionsMap.set(sessionData.id, session);
  ...
  private async loadCurrentExtension() {
    try {
      const response = await this._sdk
        .platform()
        .get("/restapi/v1.0/account/~/extension/~", null, this.requestOptions);
  ...
  private async loadActiveCalls() {
    ...
    try {
      const response = await this._sdk
        .platform()
        .get(presenceUrl, null, this.requestOptions);
      const data = await response.json();
      if (this._accountLevel) {
