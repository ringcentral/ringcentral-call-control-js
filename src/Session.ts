...
export class Session extends EventEmitter {
  ...
  get party() {
    ...
    const activeParty = parties.find(
      (p) => p.status.code !== PartyStatusCode.disconnected,
    );
    if (activeParty) {
      return activeParty;
    }
  ...
  _updatePartiesSequence(parties: Party[] = [], sequence?: Number) {
    ...
    parties.forEach((party) => {
      if (
        !this._eventPartySequences[party.id] ||
  ...
  public onUpdated(data: SessionData) {
    ...
    partiesDiff.forEach((diff) => {
      ...
      if (diff.type === "update") {
        const oldPartyIndex = this.parties.findIndex(
          (p) => p.id === diff.party.id,
        );
  ...
  async createRecord() {
    ...
    const recordings = (this.party.recordings || []).filter(
      (r) => r.id !== recording.id,
    );
